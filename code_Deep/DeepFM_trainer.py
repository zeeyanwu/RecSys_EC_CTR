...
class DeepFMTrainer:
    ...
    def train_epoch(self,train_loader):
        ...
        for features, labels in tqdm(train_loader):
            ...
            preds = self.model.forward(features)
            loss = self.loss_fn(preds.flatten(), labels)
            loss.backward()
            self.optimizer.step()
    ...
    def save_model(self):
        ...
        torch.save(self.model.state_dict(), os.path.join(self.model_save_path, "deepfm_best.pth"))

    def load_model(self):
        self.model.load_state_dict(
            torch.load("../model/deepfm_best.pth"))
...
if __name__ == "__main__":
    ...
    test_dataset = DeepFMDataset(test_df)
    train_dataloader = DataLoader(train_dataset,batch_size=batch_size,shuffle=True,num_workers=2,
persistent_workers=False)
    test_dataloader = DataLoader(test_dataset,batch_size=batch_size,shuffle=True,num_workers=2,persistent_workers=False)

    # Model
    model = DeepFM(feature_num,embedding_size,hidden_dims,dropout=0.2).to(device)
    loss_fn = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)

    # # Trainer
    trainer = DeepFMTrainer(model, loss_fn, optimizer, device=device)
    #
    # Run
