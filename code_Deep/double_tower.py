...
from Dssm_trainer import DSSMDataset
...
class DSSMTrainer:
    ...
    def get_embeddings(self, features):
        """Return user and item embeddings for recall"""
        self.model.eval()
        with torch.no_grad():
            _, user_emb, item_emb = self.model(features.to(self.device))
...
if __name__ == "__main__":
    ...
    trainer.save_model("dssm_model.pt")

    # later in online recall system:
    trainer.load_model("dssm_model.pt")
    sample = torch.randint(0, 100, (5, 8))
    user_emb, item_emb = trainer.get_embeddings(sample)
