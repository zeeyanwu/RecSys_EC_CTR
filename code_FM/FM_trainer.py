"""
Model definition → nn.Module (FM layer + wrapper).
Dataset & DataLoader → custom Dataset + torch.utils.data.DataLoader.
Training pipeline → standalone functions or a Trainer class.
"""
...
class RecSysDataset(Dataset):
    """
    CSV/NumPy based dataset for recommendation system.
    Expects columns: uid, iid, ctr, utag1, utag2, itag1, itag2, itag3
    """
...
class FMTrainer:
    ...
    def train_epoch(self, dataloader):
        ...
        for features, labels in tqdm(dataloader):
            ...
            outputs,_,_ = self.model.forward(features)
            ...
            total_correct += ((preds > 0.5).int() == labels.int()).sum().item()
    ...
    def save_model(self):
        ...
        torch.save(self.model.state_dict(), os.path.join(self.model_save_path, "fm_final.pth"))

    def load_model(self):
        self.model.load_state_dict(
            torch.load("../model/fm_final.pth"))
    ...
    def run(self, train_loader=None, val_loader=None, epochs=1, mode="train_and_eval"):
        for epoch in range(epochs):
            ...
            if mode in ["train_and_eval", "train_only"]:
                ...
                avg_loss, train_acc = self.train_epoch(train_loader)
...
if __name__ == "__main__":
    ...
    # checkpoint_dir = os.path.join("..", "model", "fm_package")
